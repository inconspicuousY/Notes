...
| 数据类型                  | 内存空间                                                     | 取值范围                                                     |
| ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| int                       | 一般情况下占用一个字长；但是当计算机的字长小于16位时，就需要占用更多个字长。换句话说，就是最少就是16位； | 最小占用16位内存空间，所以最小的取值范围为 -32767~32767，随着计算机的字长越大，取值范围也就越大。 |
...
>
> 6、在当前64位操作系统中，int类型并不是一定表示一个步长也就是64位，一般情况int占用的都是32位。在C语言中只规定了int最小的取值范围，并没有规定最大的取值范围，但是实现C语言的编译器以多数都将int编译为 16或者32位；换句话说，int类型的数据的宽度要么和short一致，要么就是和long保持一致。
