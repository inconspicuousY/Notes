...
| 数据类型                  | 内存空间                                                     | 取值范围                                                     |
| ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| int                       | 一般情况下占用一个字长；但是当计算机的字长小于16位时，就需要占用更多个字长。换句话说，就是最少就是16位；目前的计算机一般都是32位或者是64位，所以一般存储int型整数占用32位或者64位。 | 最小占用16位内存空间，所以最小的取值范围为 -32767~32767，随着计算机的字长越大，取值范围也就越大。 |
...
> 注意：
>
> 1、虽然char型数据只占用8位存储单元，但是由于计算机底层用整数存储的char型数据， 所以实际占用的是计算机对应int类型数据的存储单元，只是取值的时候，只会取存储单元中最后的8位二进制数。
>
...
| 数据类型    | 有效数字                                                     | 取值范围                    |
| ----------- | ------------------------------------------------------------ | --------------------------- |
| float       | 必须至少能表示6位有效数字。（float类型必须至少精确表示小数点后的6位有效数字） | 取值范围至少是10^-37~10^+37 |
| double      |                                                              |                             |
| long double |                                                              |                             |
